...
public class DrawerAdapter extends ArrayAdapter<RosterItem> {
	...
	public void update() {
        ...
        ip = service.getIconPicker();
        if (ip == null) {
            ip = new IconPicker(activity);
        }
		clear();
	...
	public View getView(int position, View v, ViewGroup parent) {
        ...
        if (ri.isGroup()) {
            ...
            if (v == null || v.getTag() == null || v.getTag() instanceof Holders.ItemHolder) {
                ...
                v.setBackgroundColor(Colors.DRAWER_GROUP_BACKGROUND);
                v.setTag(holder);
            } else {
            ...
            if (getCount() - 1 < 1) {
                holder.text.setText(activity.getString(R.string.NoChats));
            } else {
            ...
            if(v == null || v.getTag() == null || v.getTag() instanceof Holders.GroupHolder) {
                ...
                holder.status.setTextColor(Colors.DRAWER_SECONDARY_TEXT);
            ...
            if (roster != null) {
                state = roster.getChatState(jid);
            }
            if (state != null && state == ChatState.composing) holder.name.setTextColor(Colors.HIGHLIGHT_TEXT);
            else if (service.isHighlight(account, jid)) {
                holder.name.setTextColor(Colors.HIGHLIGHT_TEXT);
                holder.counter.setTextColor(Colors.HIGHLIGHT_TEXT);
            } else {
                holder.name.setTextColor(Colors.DRAWER_ENTRY_FOREGROUND);
                holder.counter.setTextColor(Colors.DRAWER_ENTRY_FOREGROUND);
            }
            ...
            if (!ri.isMuc()) {
                if (service.getJoinedConferences(account).containsKey(StringUtils.parseBareAddress(jid)))
