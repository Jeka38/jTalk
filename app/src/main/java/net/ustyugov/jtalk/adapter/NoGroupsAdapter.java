...
public class NoGroupsAdapter extends ArrayAdapter<RosterItem> {
	...
	public void update() {
		...
		Cursor cursor = null;
		if (service.isAuthenticated()) cursor = service.getContentResolver().query(JTalkProvider.ACCOUNT_URI, null, AccountDbHelper.ENABLED + " = '1'", null, null);
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (item.isGroup()) {
			...
			else Avatars.loadAvatar(activity, StringUtils.parseBareAddress(jid), holder.avatar);
			...
			if (service.isHighlight(account, name)) {
				...
				holder.counter.setTextColor(Colors.ENTRY_FOREGROUND);
			}
			...
			Avatars.loadAvatar(activity, name, holder.avatar);
