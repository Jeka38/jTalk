...
import org.jivesoftware.smackx.sm.Enabled;
import org.jivesoftware.smackx.sm.Resume;
import org.jivesoftware.smackx.sm.Resumed;
import org.jivesoftware.smackx.sm.R;
import org.jivesoftware.smackx.sm.A;
...
public class PacketParserUtils {
    ...
    public static Packet parseEnabled(XmlPullParser parser) throws Exception {
        Enabled enabled = new Enabled();
        boolean done = false;
        int eventType = parser.getEventType();
        while (!done) {
            if (eventType == XmlPullParser.START_TAG) {
                if (parser.getName().equals(enabled.getElementName())) {
                    for (int i = 0; i < parser.getAttributeCount(); i++) {
                        if (parser.getAttributeName(i).equals("id")) {
                            enabled.setId(parser.getAttributeValue(i));
                        } else if (parser.getAttributeName(i).equals("location")) {
                            enabled.setLocation(parser.getAttributeValue(i));
                        } else if (parser.getAttributeName(i).equals("max")) {
                            enabled.setMax(Integer.parseInt(parser.getAttributeValue(i)));
                        } else if (parser.getAttributeName(i).equals("resume")) {
                            enabled.setResume(Boolean.parseBoolean(parser.getAttributeValue(i)));
                        }
                    }
                }
            }
    ...
    public static Packet parseEnable(XmlPullParser parser) throws Exception {
        Enable enable = new Enable();
        boolean done = false;
        int eventType = parser.getEventType();
        while (!done) {
            if (eventType == XmlPullParser.START_TAG) {
                if (parser.getName().equals(enable.getElementName())) {
                    for (int i = 0; i < parser.getAttributeCount(); i++) {
                        if (parser.getAttributeName(i).equals("max")) {
                            enable.setMax(Integer.parseInt(parser.getAttributeValue(i)));
                        } else if (parser.getAttributeName(i).equals("resume")) {
                            enable.setResume(Boolean.parseBoolean(parser.getAttributeValue(i)));
                        }
                    }
                }
            }
    ...
    public static Packet parseResume(XmlPullParser parser) throws Exception {
        Resume resume = new Resume();
        boolean done = false;
        int eventType = parser.getEventType();
        while (!done) {
            if (eventType == XmlPullParser.START_TAG) {
                if (parser.getName().equals(resume.getElementName())) {
                    for (int i = 0; i < parser.getAttributeCount(); i++) {
                        if (parser.getAttributeName(i).equals("h")) {
                            resume.setH(Integer.parseInt(parser.getAttributeValue(i)));
                        } else if (parser.getAttributeName(i).equals("previd")) {
                            resume.setPrevid(parser.getAttributeValue(i));
                        }
                    }
                }
            }
    ...
    public static Packet parseResumed(XmlPullParser parser) throws Exception {
        Resumed resumed = new Resumed();
        boolean done = false;
        int eventType = parser.getEventType();
        while (!done) {
            if (eventType == XmlPullParser.START_TAG) {
                if (parser.getName().equals(resumed.getElementName())) {
                    for (int i = 0; i < parser.getAttributeCount(); i++) {
                        if (parser.getAttributeName(i).equals("h")) {
                            resumed.setH(Integer.parseInt(parser.getAttributeValue(i)));
                        } else if (parser.getAttributeName(i).equals("previd")) {
                            resumed.setPrevid(parser.getAttributeValue(i));
                        }
                    }
                }
            }
    ...
    public static Packet parseR(XmlPullParser parser) throws Exception {
        return new R();
    }

    public static Packet parseA(XmlPullParser parser) throws Exception {
        A a = new A();
        boolean done = false;
        int eventType = parser.getEventType();
        while (!done) {
            if (eventType == XmlPullParser.START_TAG) {
                if (parser.getName().equals(a.getElementName())) {
                    for (int i = 0; i < parser.getAttributeCount(); i++) {
                        if (parser.getAttributeName(i).equals("h")) {
                            a.setH(Integer.parseInt(parser.getAttributeValue(i)));
                        }
                    }
                }
            }
                ...
                // this element name and namespace.
                    ...
                    if (provider != null) {
                        if (provider instanceof IQProvider) {
                            ...
                            if (iqPacket.getType() == IQ.Type.RESULT) {
